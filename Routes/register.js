...
router.route('/')
    .post(async(req, res)=>{
       ...
       const code = `${split_type}${split_type}${split_name}${random}`;
        console.log(code);
       try{
            await connectDB.query(`INSERT INTO company (company_code, company_type, company_name, status, country, email) VALUES('${code}', '${type}', '${name}','new', '${country}', ${email})`, (err, result) =>{
                if(err){
